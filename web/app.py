...
PROVIDER_CONFIGS = {
    ...
    LLMProvider.OLLAMA: ProviderConfig(
        ...
        models=["llama3.2", "llama3.1", "mistral", "mixtral", "phi3", "gemma2", "qwen2.5"],
        requires_api_key=False,
...
async def call_llm_with_retry(client, system_prompt: str, user_content: str, model: str,
    ...
    while attempt <= max_retries:
        ...
        try:
            ...
            if ErrorClassifier.is_empty_result(output) and attempt < max_retries:
...
with st.sidebar:
    ...
    if selected_provider == LLMProvider.CUSTOM:
        model_name = st.text_input(
            "Model Name",
            value=st.session_state.get("custom_model", "custom-model"),
            key="custom_model",
            on_change=lambda: save_setting("custom_model")
        )
    else:
        # Get saved model or default
        saved_model = st.session_state.get("model_name", provider_config.models[0])
        model_options = provider_config.models + ["Custom..."]
        default_model_idx = model_options.index(saved_model) if saved_model in model_options else 0
        ...
        if model_choice == "Custom...":
            model_name = st.text_input(
                ...
                on_change=lambda: save_setting("custom_model_input")
            )
        else:
...
if dataset_mode == "Transform Existing Dataset":
    ...
    if uploaded_file:
        # Load based on file type with error handling
        ...
        if run_test or run_full:
            ...
            # Effective JSON mode (skip for local providers)
...
4. Do not include any explanation, just the JSON object"""
        ...
        supports_json = selected_provider not in [LLMProvider.LM_STUDIO, LLMProvider.OLLAMA, LLMProvider.CUSTOM]
